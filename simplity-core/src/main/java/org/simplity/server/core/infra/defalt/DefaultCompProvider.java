...
import org.simplity.server.core.service.FormService;
...
public class DefaultCompProvider implements CompProvider {
	...
	public DefaultCompProvider(final String rootPackageName) {
		 ...
		 */
		this.services.put(Conventions.App.SERVICE_FORM, FormService.getInstance());
	...
	public Service getService(final String serviceId, final ServiceContext ctx) {
		...
		/*
		 * look for a class implementing this service as per our naming convention
		 */
	...
	private Form<?> loadForm(final String formId) {
		...
		try {
			...
			logger.info("No form named {} exist. class '{}' not found", formId, cls);
			return null;
		} catch (final Exception e) {
