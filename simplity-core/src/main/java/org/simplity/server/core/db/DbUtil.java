...
public class DbUtil {
	...
	private static boolean useIntForBoolean = true;

	/**
	 *
	 * @param useInt true to use int (1/0) for boolean fields in DB. false to use
	 *               true/false
	 */
	public static final void setUseIntForBoolean(boolean useInt) {
		useIntForBoolean = useInt;
	}
	...
	public static final void setPsParamValue(final PreparedStatement ps, final int position, final Object value,
		...
		case Boolean:
			...
			} else if (useIntForBoolean) {
				ps.setInt(position, (boolean) value ? 1 : 0);
			} else {
	 ...
	 * @param paramTypes can be null
	 * @return string representation
	 */
	public static String logParameters(Object[] params, ValueType[] paramTypes) {
		...
		int n = params.length;
		for (int i = 0; i < n; i++) {
			sb.append(i + 1 + " ").append(paramTypes[i]).append(":");
			if (params[i] == null) {
				sb.append("null");
			} else {
