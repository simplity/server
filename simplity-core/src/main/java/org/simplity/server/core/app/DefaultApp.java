...
class DefaultApp implements App {
	...
	DefaultApp(final AppConfig config) throws Exception {
		...
		String rootPackageName = config.appRootPackage;
		...
		if (rootPackageName == null || rootPackageName.isEmpty()) {
			...
			this.compProvider = new DefaultCompProvider(rootPackageName);
			if (this.compProvider == null) {
				throw new Exception("Error while initializing comp provider using root package " + rootPackageName);
			}
		}
