...
/**
 * utility to read meta data from database
 *
 * @author simplity.org
 *
 */
public class MetaUtil {
	...
	private static DataTable<ColumnMetaRecord> pouplateColumns(ResultSet rs) throws SQLException {
		...
		while (rs.next()) {
			...
			StringBuilder valueTypeInfo = new StringBuilder().append(typeName).append(" ").append(size);
			if (nbrDigits > 0) {
				valueTypeInfo.append(" decimals=").append(nbrDigits);
			}
			...
			column.setValueTypeInfo(valueTypeInfo.toString());
	...
	private static String toFieldName(String dbName) {
		...
		StringBuilder name = new StringBuilder().append(parts[0]);
		for (int i = 1; i < parts.length; i++) {
			String part = parts[i];
			name.append(capitalise(part));
		}
	...
	private static String toLabel(String dbName) {
		final String[] parts = dbName.split("_");
		StringBuilder name = new StringBuilder().append(capitalise(parts[0]));
		for (int i = 1; i < parts.length; i++) {
			String part = parts[i];
			name.append(' ').append(capitalise(part));
		}
		return name.toString();
	}
